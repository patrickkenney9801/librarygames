...
public abstract class BoardGame {
  ...
  public static class GameMetadata {
    ...
    public GameMetadata(
      ...
      if (playing) {
        ...
        this.userTurn =
            (currentUser.equals(user1) && player1Turn)
                || (currentUser.equals(user2) && !player1Turn);
        this.finished = winner != 0;
        this.won = false;
        if (finished) {
          boolean player1Won = winner == 1 || winner == 3;
          this.won =
              (currentUser.equals(user1) && player1Won)
                  || (currentUser.equals(user2) && !player1Won);
        }
      }
    }
  }
  ...
  public BoardGame(
    ...
    if (Game.isOnline()) {
      ...
      setPlayerIsSpectating(
          !getPlayer1().equals(Game.getPlayer().getUsername())
              && !getPlayer2().equals(Game.getPlayer().getUsername()));

      // determine whether it is the logged players turn or not
      setPlayer1Turn(moves % 2 == 0);
      setPlayerTurn(false);
      if (!isPlayerIsSpectating()
              && (getPlayer1().equals(Game.getPlayer().getUsername()) && isPlayer1Turn())
  ...
  public boolean update(
      ...
      boolean player1OnGame,
      boolean player2OnGame) {
    if (!getGameKey().equals(gameKey)) return false;
    setPenultMove(penultMove);
    setLastMove(lastMove);
    setBoard(board);
    setPieces();
    setPlayer1Turn(moves % 2 == 0);
    setPlayerTurn(
        !isPlayerIsSpectating()
                && (getPlayer1().equals(Game.getPlayer().getUsername()) && isPlayer1Turn())
            || (getPlayer2().equals(Game.getPlayer().getUsername()) && !isPlayer1Turn()));
    updateWinner(winner);
    setMoves(moves);
    if (winner == 0) setMoves(moves + 1);
    setPlayer1OnGame(player1OnGame);
    setPlayer2OnGame(player2OnGame);
  ...
  public void updateWinner(int winner) {
    setWinner(null);
    setScoreInfo(null);
    if (winner == 1 || winner == 3) setWinner(getPlayer1());
    else if (winner == 2 || winner == 4) setWinner(getPlayer2());
    if (winner == 3) setScoreInfo(getPlayer2() + " resigned");
    else if (winner == 4) setScoreInfo(getPlayer1() + " resigned");
  }
  ...
  public static String makeMove(
    ...
    switch (gameType) {
      ...
      case GO19x19:
        return Go.makeMove(board1D, isPlayer1Turn, penultMove1D, lastMove1D, moveFrom1D, moveTo1D);
      default:
  ...
  public void makeMoveOffline(int movingFrom, int movingTo) {
    ...
    if (movingTo == -2)
      if (isPlayer1Turn()) updateWinner(4);
      else updateWinner(3);
    ...
    String newBoard =
        BoardGame.makeMove(
            gameType, oldBoard, moves % 2 == 0, penultMove, lastMove, movingFrom, movingTo);
    setBoard(newBoard);

    // if the game is go
    if (getGameType() == GameType.GO9x9
        || getGameType() == GameType.GO13x13
        || getGameType() == GameType.GO19x19) {
      ...
      if (movingTo > -1) {
        ...
        for (int i = 0; i < oldBoard.length(); i++)
          if (oldBoard.charAt(i) != newBoard.charAt(i)) capturedPieces++;
        if (getMoves() % 2 == 0)
      ...
      if (movingTo == -1 && getLastMove() == -1) {
        ...
        switch (getGameType()) {
          case GO19x19:
            ((Go) this)
                .setPlayer2Score(territoryScores[1] + ((Go) this).getBlackStonesCaptured() + 6);
            break;
          case GO13x13:
            ((Go) this)
                .setPlayer2Score(territoryScores[1] + ((Go) this).getBlackStonesCaptured() + 3);
            break;
          default:
            ((Go) this)
                .setPlayer2Score(territoryScores[1] + ((Go) this).getBlackStonesCaptured() + 1);
        }
        updateWinner(1);
        if (((Go) this).getPlayer2Score() >= ((Go) this).getPlayer1Score()) updateWinner(2);
        setScoreInfo(
            "Score: "
                + ((Go) this).getPlayer1Score()
                + " - "
                + ((Go) this).getPlayer2Score()
                + ".5");
      }
    }
  ...
  protected char[][] getBoardCopy() {
    ...
    for (int i = 0; i < copy.length; i++)
      for (int j = 0; j < copy.length; j++) copy[i][j] = getBoard()[i][j];
    return copy;
  }
  ...
  protected static char[][] getPaddedBoard(String board) {
    ...
    for (int i = 0; i < padded.length - 2; i++)
      for (int j = 0; j < padded.length - 2; j++) padded[i + 1][j + 1] = reqBoard[i][j];
    return padded;
  }

  /**
   * Returns a padded copy of the board for testing or other cases The board is surrounded by null
   * characters so indexoutofboundexceptions do not occur
   *
   * @return
   */
  protected char[][] getPaddedBoardCopy() {
    ...
    for (int i = 0; i < copy.length - 2; i++)
      for (int j = 0; j < copy.length - 2; j++) copy[i + 1][j + 1] = getBoard()[i][j];
    return copy;
  }
  ...
  /**
   * This method WILL OVERWRITE a given board by replacing the given index with replaceChar as well
   * as any other connected characters of the same type
   *
   * @param board
   * @param x
   * @param y
  ...
  protected static char[][] paintBoard(char[][] board, int x, int y, char target, char replace) {
    board[x][y] = replace;
    if (board[x][y + 1] == target) board = paintBoard(board, x, y + 1, target, replace);
    if (board[x][y - 1] == target) board = paintBoard(board, x, y - 1, target, replace);
    if (board[x + 1][y] == target) board = paintBoard(board, x + 1, y, target, replace);
    if (board[x - 1][y] == target) board = paintBoard(board, x - 1, y, target, replace);
    return board;
  }
  ...
  public static String createNewBoard(GameType gameType) {
    ...
    switch (gameType) {
      case GO9x9: // handle game type Go 9x9
        for (int i = 0; i < 9 * 9; i++) board += "0";
        break;
      case GO13x13: // handle game type Go 13x13
        for (int i = 0; i < 13 * 13; i++) board += "0";
        break;
      case GO19x19: // handle game type Go 19x19
        for (int i = 0; i < 19 * 19; i++) board += "0";
        break;
  ...
  public String getBoardAsString() {
    String board = "";
    for (char[] row : getBoard()) for (char piece : row) board += piece;
    return board;
  }
